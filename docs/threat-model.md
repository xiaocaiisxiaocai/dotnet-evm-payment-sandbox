...
- The .NET solution and the test-only PaymentRouter/TestUSDC implementation, including the typed contract adapter, strict ERC-2612 construction/exact-block preflight/durable workflow, runnable SQLite-backed Payment Intent API, bounded SIWE challenge verification, chain-observation/checkpoint library, provisional reversible ledger, reversible confirmation qualification, explainable reconciliation reports, test-only transaction lifecycle, identity checks, permit, fuzz, invariant, and local deployment tests.
...
Reviewed permit/code policy -> exact-block token domain + nonce -> durable reservation -> external EOA signature -> unknown-before-release Router calldata
```
...
- ERC-2612 domain fields, owner nonce, signature, and RPC responses are untrusted allowance input. Week 18 fixes the EIP-712 message and recovered EOA. Week 19 additionally pins code/name/domain/nonce reads to one block, compares reviewed runtime/domain facts, durably reserves `(chain, token, owner, nonce)`, and commits an unknown transition before exact calldata is released. One endpoint can still lie, the signature remains front-runnable, `SubmissionAccepted` is transport-only, and nonce movement cannot identify the consuming transaction. Merchant/`PaymentId`, account transaction nonce, receipt, delivery, finality, and settlement remain separate boundaries.
...
| ERC-2612 typed data, signature, and persisted calldata | Can grant a token allowance, be copied/front-run while usable, or be replayed from a stolen database | Weeks 18-19 bind chain/token/runtime/name/domain/Router/owner/value/nonce/deadline, recover canonical low-`s` EOA signatures, reserve the nonce in a bounded shared SQLite file, persist unknown before release, and retry only exact bytes from an observed transition. String output is redacted, but the local unencrypted database contains signature-bearing calldata and is not a tamper-evident vault |
...
19. An ERC-2612 signature must recover its named EOA over the exact reviewed
    ...
    is composed into calldata. Exact-block runtime/name/domain/nonce preflight
    and durable nonce reservation must precede release; an unknown marker must
    commit before calldata escapes, and retry may release only identical bytes
...
| --- | --- | --- | --- | --- | --- | --- |
| S01 | A key, signed transaction, permit calldata, or credential enters source, history, logs, or exception output | Medium | High | Ignore rules, working-tree/history scans, canary, redacted signed-payload strings, sanitized adapter/corruption exceptions, and a non-exporting generated Anvil key with best-effort zeroing; local lifecycle and permit DBs remain sensitive and unencrypted | Partly controlled | Gates A/D |
