...
- Status: Week 11 bounded update to the Week 1 security baseline
...
- The .NET solution and the test-only PaymentRouter/TestUSDC implementation, including the typed contract adapter, runnable SQLite-backed Payment Intent API, bounded chain-observation/checkpoint library, provisional reversible ledger, reversible confirmation qualification, identity checks, permit, fuzz, invariant, and local deployment tests.
- Local Anvil, plus one later smoke test on Ethereum Sepolia.
- Protocol-native finality and reconciliation for the partial Indexer/Ledger/Finality boundary, plus the Orchestrator and SIWE components planned for later gates.
...
Future reconciliation -> explainable business decisions
...
| Payment intents, observations, checkpoints, ledger, and finality policy | Duplicate credit, stale qualification, lost entries, or unexplained differences | Separate strict schemas; atomic intent/observation/ledger behavior; exact caught-up snapshots; policy/source fingerprints; linked effect and qualification reversals; atomic retry verification; protocol finality, balances, tamper evidence, and reconciliation remain Gates B/C |
...
| S07 | Reorg, truncated logs, or incorrect finality causes false credit | Medium | High | Weeks 8-9 retain exact fork/canonicality history; Week 10 appends provisional reversals; Week 11 requires caught-up snapshots and appends qualification revocation after Ledger reversal or threshold loss; independent completeness checks, protocol finality, and authorization remain | Partly controlled | Gates B/C |
...
| S09 | SQLite, Ledger, or Finality data is modified and effects become unexplainable | Medium | Medium | Versioned migrations, `STRICT`/`CHECK`/foreign-key/trigger constraints, parameterized SQL, append-only source/effect/qualification identities, linked reversals, source/policy fingerprints, and atomic checkpoint revisions exist; backup, tamper evidence, finalized balances, and reconciliation remain | Partly controlled | Gate C |
...
- Protocol-native finality, reconciliation, Orchestrator, or signer becomes runnable, or the API is exposed beyond loopback.
