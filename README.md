...
- `PaymentSandbox.Orchestrator` reserves account nonces transactionally, persists initial/replacement attempts before broadcast, and derives lifecycle state from append-only operation, attempt, broadcast, and receipt histories.
...
- Loopback SIWE HTTP endpoints require the exact server-configured HTTPS Origin, bind each nonce to a separate 256-bit HttpOnly flow cookie, and return generic non-leaking failures.
- Successful proof creates restart-safe opaque session and CSRF cookies whose raw values are never stored; relogin rotates the old session transactionally and logout requires double-submit CSRF before one-way revocation.
- Login identity remains separate from Router payments, permits, transaction signing, roles, tenants, and authorization; the existing Payment Intent endpoints intentionally remain unauthenticated.
...
- A test-only one-shot fault observer fails an exact public action only after its successful result exists, proving post-commit retries from durable state without exposing payloads or adding production crash switches.
- Transaction create/broadcast/replacement/receipt retries avoid duplicate nonce reads, signing, broadcasting, and receipt reads; Permit reserve/prepare/outcome/terminal retries replay or no-op, while an unknown authorization requires an explicit exact-byte retry.
- SQLite test assemblies serialize independent cases because temporary cleanup clears process-wide connection pools; explicit in-test concurrency and cross-project parallel execution remain intact.
...
The default SQLite files are created under
`src/PaymentSandbox.Api/data/payment-intents.db` and
`src/PaymentSandbox.Api/data/authentication.db` and survive restart. Processes
coordinate only when configured to use the same local files. Authentication is
loopback-only and has no rate limiting or production hosting controls. Its
cookies remain `Secure`; the HTTP URL above is suitable for API/testing work,
not an ordinary browser deployment. See the Week 17 guide before changing the
HTTPS/same-site boundary.
...
| `tests/PaymentSandbox.Testing/`            | Shared test-only deterministic post-completion fault injection; no production project references it.                  |
...
| `src/PaymentSandbox.Api/`                  | Runnable local Payment Intent API plus loopback SIWE/session HTTP and independently migrated SQLite stores.              |
| `tests/PaymentSandbox.Api.Tests/`          | Service, migration, constraint, restart, real Kestrel, cookie/origin/CSRF, and concurrency tests.                        |
...
Week 13 adds `PaymentSandbox.Orchestrator` as a class library, not a hosted
wallet. It binds an immutable Router payment request to one locally reserved
account nonce, persists signed attempts before broadcast, resends the exact same
bytes after an unknown transport result, and permits only bounded fee-only
replacements. Operation, attempt, broadcast, and receipt facts are append-only;
the public state is derived from that history. Accepted evidence dominates a
later ambiguous response for the same bytes. A receipt remains an observation,
...
Week 15 adds `PaymentSandbox.Authentication` as another independent class
library. It supports one canonical EOA-only ERC-4361 subset: a configured HTTPS
origin and same-origin URI, EIP-55 address, fixed statement, version 1,
Anvil/Sepolia chain, CSPRNG nonce, issued/expiry times, and ERC-191 recovery. The
in-memory store atomically consumes the exact server challenge once; 24
concurrent replays yield one success. The result is not a session or
...
Week 17 composes that proof into four loopback authentication endpoints. A
separate HttpOnly flow secret binds challenge delivery to one browser context;
verification creates hashed opaque session/CSRF credentials; relogin rotates a
prior session in the same transaction as replacement; and double-submit CSRF
guards one-way logout revocation. Exact configured Origin, hardened `__Host-`
cookies, restart, duplicate-cookie, expiry, and generic-failure behavior are
exercised through real Kestrel. The Payment Intent API is still not authorized
by SIWE. See the [Week 17 loopback session guide](docs/learning/week-17-loopback-siwe-sessions.md).

Week 18 adds a separate allowance-signature boundary. A reviewed policy fixes
the EIP-712 token domain, Router spender, chain, and lifetime; an explicit token
nonce and exact raw amount produce deterministic wallet JSON and hashes. The
library verifies canonical low-`s` EOA signatures before preparing unsigned
`payWithPermit` calldata for the matching verified Router and required owner
...
Week 21 uses that operation boundary as a deterministic test-only fault seam.
One selected successful action throws after its business result exists but
before the caller receives it, modeling a committed mutation whose response is
lost. Recovery tests prove durable replay/no-work behavior and exact dependency
call counts across transaction create, broadcast, replacement, receipt refresh,
and Permit reserve, preparation, authorization, outcome, and expiry paths. They
also close exact replacement and Permit outcome idempotency gaps. No production
crash switch, chaos service, retry worker, or runbook is added. See the
[Week 21 fault-recovery guide](docs/learning/week-21-deterministic-fault-recovery.md).
...
The 2026-08-30 Week 16 committed-snapshot verification passed 284/284 .NET
tests, including 49/49 focused Authentication tests. All 36 unchanged Foundry
tests, the 1,030-byte/zero-slot Router baseline, and deployment plus the signed
...
[`33306985314`](https://github.com/xiaocaiisxiaocai/dotnet-evm-payment-sandbox/actions/runs/33306985314)
...
working-tree/complete 44-commit history scans found no leaks. GitHub Actions run
[`33309330651`](https://github.com/xiaocaiisxiaocai/dotnet-evm-payment-sandbox/actions/runs/33309330651)
then independently passed locked .NET, Foundry plus signed RPC replay, and
secret-scan jobs.

The 2026-08-30 Week 19 clean committed snapshot at implementation commit
...
Authentication tests, and 45/45 API tests. All 36 unchanged Foundry tests, the
1,030-byte/zero-slot Router baseline, and deployment plus the signed Anvil
lifecycle from 1,294 tracked files passed. The dynamic canary and
working-tree/complete 47-commit history scans found no leaks. GitHub Actions run
[`33312644170`](https://github.com/xiaocaiisxiaocai/dotnet-evm-payment-sandbox/actions/runs/33312644170)
then independently passed locked .NET, Foundry plus signed RPC replay, and
secret-scan jobs.

The 2026-08-30 Week 20 clean committed snapshot at implementation commit
`8b136b4` passed 339/339 .NET tests, including 4/4 focused Observability,
43/43 Orchestrator, and 32/32 Permit tests. All 36 unchanged Foundry tests, the
1,030-byte/zero-slot Router baseline, and deployment plus the signed Anvil
lifecycle from 1,306 tracked files passed. The dynamic canary and
working-tree/complete 50-commit history scans found no leaks. GitHub Actions run
...
records the strict SIWE boundary. GitHub Actions run
...
| Week 21            | **Complete:** deterministic post-commit response-loss injection plus durable replay/no-work and side-effect-count recovery evidence.                                              |
| Weeks 22-24        | Add runbooks, security review, portfolio evidence, and a reproducible `v1.0.0` sample release.                                                                                      |
